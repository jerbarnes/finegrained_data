...
if __name__ == "__main__":
    ...
    processed = {"services": {"train": [], "dev": [], "test": []},
                 "universities": {"train": [], "dev": [], "test": []}
                 }
    ...
    for corpus in ["services", "universities"]:
        ...
        if "basedata" not in os.listdir(os.path.join(basedir, corpus)):
            for subdir in [i for i in os.listdir(os.path.join(basedir, corpus))]:
                ...
                #break into train, dev, test splits
                train_idx = int(len(ff) * .8)
                dev_idx = int(len(ff) * .9)
                train = ff[:train_idx]
                dev = ff[train_idx:dev_idx]
                test = ff[dev_idx:]
                for subname, subcorpus in [("train", train), ("dev", dev), ("test", test)]:

                    for bfile, mfile in subcorpus:
                        ...
                        if len(o["opinions"]) > 0:
                            ...
                            processed[corpus][subname].extend(sentence_anns)
    ...
    for cname, corpus in processed.items():
        for subname, subcorpus in corpus.items():
            os.makedirs(os.path.join("../processed", "darmstadt", cname), exist_ok=True)
            with open(os.path.join("../processed", "darmstadt", cname, "{0}.json".format(subname)), "w") as out:
